...
class YoutubeScraper {
  ...
  static parse_video(videoRenderer, currentTime) {
    ...
    if (
      videoRenderer?.thumbnailOverlays[0]?.thumbnailOverlayTimeStatusRenderer?.text
        ?.simpleText === "SHORTS"
    ) {
      const lengthData = this.parseShortsLength(
      ...
      video_entry.timeText =
        videoRenderer.thumbnailOverlays[0].thumbnailOverlayTimeStatusRenderer.text.simpleText;
      video_entry.lengthSeconds = this.calculate_length_in_seconds(
        video_entry.timeText
      );
    }
    video_entry.videoThumbnails = this.extract_thumbnail_data(
      video_entry.videoId
    );
    if ("ownerBadges" in videoRenderer) {
      video_entry.isVerified =
        videoRenderer.ownerBadges[0].metadataBadgeRenderer.style ===
        "BADGE_STYLE_TYPE_VERIFIED";
      video_entry.isVerifiedArtist =
        videoRenderer.ownerBadges[0].metadataBadgeRenderer.style ===
  ...
  static calculate_length_in_seconds(lengthText) {
    ...
    for (let i = hours_minutes_seconds.length - 1; i >= 0; i--) {
      length_seconds +=
        Math.pow(60, hours_minutes_seconds.length - i - 1) *
        hours_minutes_seconds[i];
    }
  ...
  static extract_thumbnail_data(videoId) {
    ...
    return [
      this.create_thumbnail_dictionary(
        "maxres",
        `https://i.ytimg.com/vi/${videoId}/maxres.jpg`,
        1280,
        720
      ),
      this.create_thumbnail_dictionary(
  ...
  static parseShortsLength(accessibilityData) {
    ...
    let lengthSeconds = 0;
    const shortsRegex =
      /(months?|years?|days?|hours?|weeks?) ago (\d*) (second|minute)/;
